 ...
 */
import {
  DEFAULT_TOLERATED_FAILURE_RATIO, buildDays, currentState, measuredWindowDays, overallState,
  statusDayIndex, uptime,
} from './aggregate.mjs'
...
export function buildSnapshot(store, config, { now = Date.now() } = {}) {
  ...
  const todayIndex = statusDayIndex(now)
  /** The first GMT+8 day a component was probed, or null when it never was. */
  const firstDayIndex = (probes) => {
    let first = null
    for (const probe of probes) {
      const idx = statusDayIndex(probe.at)
  ...
  const components = COMPONENTS.map((component) => {
    ...
    // How much of the window this row actually covers. A row whose first reading is
    // later than the page's must say its own span, not the page's.
    const windowDays = measuredWindowDays(firstDayIndex(probes), todayIndex, config.historyDays)
  ...
  const firstIndexes = COMPONENTS
    .map((component) => firstDayIndex(byComponent.get(component.id) ?? []))
    .filter((index) => index !== null)
  const windowDays = firstIndexes.length > 0
    ? measuredWindowDays(Math.min(...firstIndexes), todayIndex, config.historyDays)
    : 0
  ...
  return {
    ...
    mergedDays,
    incidents,
    guardian: buildGuardian(store, config, {
      now,
      // The same probes `availability` is computed from, so the number and its label
      // cannot cover different periods.
      availabilityDays: measuredWindowDays(
        firstDayIndex(byComponent.get(HEALTH_COMPONENT) ?? []), todayIndex, config.historyDays,
      ),
    }),
  }
}
 ...
 */
function buildGuardian(store, config, { now, availabilityDays = 0 }) {
  const accounts = store.latestMetric('hrt.accounts')
  const doses = store.latestMetric('hrt.doses')
  ...
  return {
    ...
    // Just the window. The filtering that produces these numbers is an
    // implementation detail of reading them — see CF_HUMAN_FILTER in probe.mjs —
    // and the tile says 访问量 rather than 访客, so it describes itself without a
    // footnote about what was taken out.
    ...
    availabilityDays,
    // The span comes from the probes behind the figure, not from the configured
    // window: a service four days old must not print "最近 90 天".
