...
/**
 * The `curl:` line from a node's report, if it has one.
 *
 * Only ever called for a row already judged failed, and only to make the stored
 * `error` say something a reader can act on. A node that cannot connect leaves
 * `error` empty, so without this the page would show a bare "unknown" for the most
 * interesting case there is — a carrier that can't reach the site.
 *
 * Deliberately shaped to *find* a reason, never to decide one: the calibration run
 * proved `report_source` can contain alarming text on a healthy response.
 */
function curlReason(row) {
  const report = String(row.report_source ?? '')
  const match = report.match(/^\s*curl:\s*\(\d+\)[^\n]*/m)
