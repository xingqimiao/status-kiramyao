...
export async function readCloudflareVisits(config, { fetchImpl = fetch, now = Date.now() } = {}) {
  ...
  // Hosts carry a port in this zone, so the wildcard suffix is what actually matches.
  const query = `query {
    viewer {
      zones(filter: { zoneTag: ${JSON.stringify(zoneTag)} }) {
        httpRequestsAdaptiveGroups(
          limit: 100
          filter: { datetime_geq: ${JSON.stringify(since)}, datetime_leq: ${JSON.stringify(until)} }
          orderBy: [count_DESC]
        ) {
          count
          sum { visits }
