...
export function loadConfig(env = process.env, { rootDir = process.cwd() } = {}) {
  ...
  return {
    ...
    boce: {
      ...
      /**
       * When the daily CN sample runs: local hour of day, 0..23. Default midnight.
       *
       * An hour rather than an interval because an interval drifts. The old
       * `BOCE_INTERVAL_HOURS` counted from whenever the process happened to boot, so
       * the sample wandered around the clock and its "one per day" promise was only
       * true if the process never restarted — see the note on `boceRoundIfStale` in
       * server.mjs for how a restart turned that into a day with no sample at all.
       *
       * `intervalHours` is kept for the *display* cadence (staleness windows, the
       * 「每 N 小时采样」 copy), derived from 24 so it stays correct.
       */
      hourOfDay: num('BOCE_HOUR_OF_DAY', 0),
      /**
       * The minute past that hour. Five by default, not zero.
       *
       * The sample used to run at 00:00, and the effect was that the day opened with
       * no CN reading: the newest sample was yesterday's, so today's cell was grey
       * until the round landed. Five minutes is inside the day it belongs to, which is
       * what makes the row refresh "with the others" instead of a third of the way
       * through the UTC day. Costs nothing — it is the same single sample.
       */
      minuteOfHour: num('BOCE_MINUTE_OF_HOUR', 5),
