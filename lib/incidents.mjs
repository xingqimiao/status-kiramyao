 ...
 * service failure.
 */
export function deriveIncidents(probes, opts = {}) {
  return incidentsFromRuns(toRuns(probes ?? [], opts.gapMs ?? DEFAULT_GAP_MS), opts)
}

/**
 * The same rule, over runs that were already collapsed.
 *
 * The Worker's D1 path collapses runs in SQL (a 90-day window of healthy probes is one
 * run, not 13,000 rows) and hands the result here. Keeping one implementation means the
 * D1 incidents and the SQLite incidents cannot drift — `worker/test/port.test.mjs` seeds
 * the same data through both and asserts they agree.
 */
export function incidentsFromRuns(runs, {
  component,
