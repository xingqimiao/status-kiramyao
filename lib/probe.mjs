 ...
 * a number worth showing and one that is mostly noise.
 *
 * Both terms come from inspecting what this zone actually recorded, not from a guess:
 *
 *   - **`verifiedBotCategory: ""`** drops Cloudflare-classified crawlers. In a 20-hour
 *     window the zone logged 73 visits from "Search Engine Crawler", plus AI crawlers
 *     and SEO tools. A crawl is not a reader.
 *   - **`userAgentBrowser_neq: "Curl"`** drops command-line clients, and this one
 *     mattered more than expected: 81 of hrt.kiramyao.com's 286 visits arrived that
 *     way. Inspecting them showed `curl/8.21.0` fetching /privacy 38 times and / 10
 *     times — which is *our own verification traffic* from the session that built this,
 *     plus the monitoring loop. A page that counts its own health checks as visitors is
 *     flattering itself.
 *
 * `ChromeHeadless` is not excluded, though it is also automated: it is a real rendering
 * engine, it could be a legitimate preview or screenshot service, and the volume here
 * was 52 requests. Excluding it would be a guess in the other direction.
 *
 * Deliberately NOT a whitelist of browser names. The zone's largest `userAgentBrowser`
 * bucket is "Unknown" (5669 requests, versus 1512 for Chrome), and a whitelist drops all
 * of it — much of which is likely real traffic whose agent string the classifier could
 * not place. That would undercount far more than this overcounts.
 */
const CF_HUMAN_FILTER = 'verifiedBotCategory: "", userAgentBrowser_neq: "Curl"'
