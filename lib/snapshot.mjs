...
function buildGuardian(store, config, { now }) {
  ...
  return {
    ...
    visitsHint: config.cloudflare
      // Says what is excluded, because "visits" without that reads as "people",
      // and Cloudflare cannot give people on this plan. See CF_HUMAN_FILTER.
      ? '最近 24 小时，已排除爬虫与命令行访问'
      : '',
